...
const previewManagedSettings = Effect.fn('syncFlow.previewManagedSettings')(function* () {
...
export const runManagedSkillsSync = Effect.fn('syncFlow.runManagedSkillsSync')(function* (
  dry: boolean,
) {
...
export const runManagedSettingsSync = Effect.fn('syncFlow.runManagedSettingsSync')(function* (
  dry: boolean,
) {
  if (dry) {
  ...
  Effect.gen(function* () {
    ...
    if (dry) {
      if (result.conflicts.length > 0) {
        yield* Console.log(`Would encounter ${result.conflicts.length} conflict(s):`);
...
export const runStowSync = Effect.fn('syncFlow.runStowSync')(function* (dry: boolean) {
...
export const runFullSync = Effect.fn('syncFlow.runFullSync')(function* (dry: boolean) {
