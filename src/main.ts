...
const ManagedAiLayer = Layer.mergeAll(
  ClaudeSettings.Live.pipe(Layer.provideMerge(StateLayer), Layer.provideMerge(PlatformLayer)),
  CodexSettings.Live.pipe(Layer.provideMerge(StateLayer), Layer.provideMerge(PlatformLayer)),
  AiSkills.Live.pipe(Layer.provideMerge(StateLayer), Layer.provideMerge(PlatformLayer)),
);
...
const MainLayer = Layer.mergeAll(PlatformLayer, StateLayer, ManagedAiLayer, RuntimeLayer);

BunRuntime.runMain(Command.run(dot, { version: '0.1.0' }).pipe(Effect.provide(MainLayer)));
