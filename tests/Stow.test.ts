...
const mockFileSystem = (state: FsState) =>
  FileSystem.layerNoop({
    ...
    readLink: (path) => {
      ...
      return target ? Effect.succeed(target) : Effect.fail(notFound('readLink', path));
    },
    rename: (from, to) =>
      Effect.sync(() => {
    ...
    readDirectory: (path) => {
      ...
      return contents ? Effect.succeed(contents) : Effect.fail(notFound('readDirectory', path));
    },
  });
...
* cannot stow .zshrc over existing target .zshrc since it is a directory`,
    ...
    it.effect('returns empty conflicts on clean stderr', () =>
      ...
      }).pipe(Effect.provide(makeTestLayer(0, 'LINK: .bashrc => home/.bashrc', makeFsState()))),
    );
  ...
  describe('sync', () => {
    it.effect('returns parsed links on exit code 0', () =>
      Effect.gen(function* () {
        ...
        expect(links[0]).toEqual(new StowLink({ target: '.bashrc', source: 'home/.bashrc' }));
        expect(links[1]).toEqual(new StowLink({ target: '.zshrc', source: 'home/.zshrc' }));
      }).pipe(
        Effect.provide(
          makeTestLayer(
            0,
    ...
    it.effect('passes --no-folding to stow sync', () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(commands).toEqual([['stow', '--no-folding', '-v', 'home', '-t', '/test/home']]);
      }).pipe(
        Effect.provide(
          Stow.Live.pipe(
            Layer.provideMerge(recordingSpawner),
  ...
  describe('resolveConflicts', () => {
    ...
    it.effect('delete: removes files, returns Resolved with resolutions', () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(fsState.removed).toEqual(['/test/home/.bashrc', '/test/home/.zshrc']);
  ...
  describe('checkAddable', () => {
    ...
    it.effect('absolute path -> InvalidPath', () => {
      ...
      return Effect.gen(function* () {
        const stow = yield* Stow;
        const result = yield* stow.checkAddable('/etc/passwd').pipe(Effect.flip);
    ...
    it.effect('path with .. -> InvalidPath', () => {
      ...
      return Effect.gen(function* () {
        const stow = yield* Stow;
        const result = yield* stow.checkAddable('.config/../../../etc/passwd').pipe(Effect.flip);
  ...
  describe('removeDotfile', () => {
    ...
    it.effect('propagates NotManaged', () => {
      ...
      return Effect.gen(function* () {
        const stow = yield* Stow;
        const result = yield* stow.removeDotfile('.nonexistent').pipe(Effect.flip);
