...
const makeTestLayer = (files: FsFiles) => {
  ...
  return CodexSettingsLive.pipe(Layer.provideMerge(stateLayers), Layer.provideMerge(baseLayer));
};
...
`.trimStart(),
      ...
      return Effect.gen(function* () {
        const settings = yield* CodexSettings;
        const machineLocalError = yield* settings.ignore('projects').pipe(Effect.flip);
