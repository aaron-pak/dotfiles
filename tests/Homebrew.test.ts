...
const makeTestLayer = (exitCode: number, stdout: string, stderr: string, fsState: FsState) =>
  Homebrew.Live.pipe(
    Layer.provideMerge(makeMockChildProcessSpawner({ exitCode, stdout, stderr })),
...
describe('Homebrew service', () => {
  describe('checkInstalled', () => {
    it.effect('returns true when brew is found', () =>
      ...
      }).pipe(Effect.provide(makeTestLayer(0, '/opt/homebrew/bin/brew', '', makeFsState()))),
    );
  ...
  describe('install', () => {
    it.effect('succeeds on exit code 0', () =>
      ...
      }).pipe(Effect.provide(makeTestLayer(0, '==> Installation successful!', '', makeFsState()))),
    );
    ...
    it.effect('fails with HomebrewInstallError on non-zero exit', () =>
      ...
      }).pipe(Effect.provide(makeTestLayer(1, '', 'Installation failed', makeFsState()))),
    );
  });
  ...
  describe('bundleDryRun', () => {
    ...
    it.effect('returns satisfied when all packages installed', () => {
      ...
      return Effect.gen(function* () {
        ...
        Effect.provide(makeTestLayer(0, "The Brewfile's dependencies are satisfied.", '', fsState)),
      );
    });
...
→ Cask ghostty needs to be installed or updated.`;
      ...
      return Effect.gen(function* () {
        ...
        expect(result.missing[0]).toEqual(new InstalledPackage({ name: 'stow', type: 'formula' }));
        expect(result.missing[1]).toEqual(new InstalledPackage({ name: 'ghostty', type: 'cask' }));
