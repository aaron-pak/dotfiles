...
import { aiHelpText, runAiHubWithHooks, runSkillsManagerWithHooks } from '../src/commands/ai.js';
import { AiSkills, type UnmanageDisposition } from '../src/services/AiSkills.js';
...
const makeAiSkillsLayer = (
  ...
  Layer.succeed(AiSkills, {
      ...
      Effect.gen(function* () {
        ...
        yield* Ref.update(entries, (currentEntries) =>
          currentEntries.map((entry) =>
            names.includes(entry.name) ? { ...entry, targets: nextTargets } : entry,
          ),
        );
    ...
    unmanageMany: (
      ...
      Effect.gen(function* () {
        if (unmanageDispositions !== undefined) {
          yield* Ref.update(unmanageDispositions, (current) => [...current, disposition]);
        }
    ...
    list: () => Ref.get(entries).pipe(Effect.map((currentEntries) => [...currentEntries])),
  });
...
describe('ai interactive flows', () => {
  ...
  it.effect('runAiHubWithHooks routes to the skills manager', () =>
    Effect.gen(function* () {
      ...
      yield* runAiHubWithHooks({
        selectAssetKind: () =>
          Ref.modify(calls, (currentCalls) => {
            ...
            const selection: 'skills' | 'exit' = currentCalls.length === 0 ? 'skills' : 'exit';
      ...
      expect(yield* Ref.get(calls)).toEqual(['select-asset', 'run-skills', 'select-asset']);
    }),
  );
      ...
      Effect.gen(function* () {
        ...
        yield* runSkillsManagerWithHooks({
          selectSkills: (_entries, selectedNames) =>
            Effect.gen(function* () {
              yield* Ref.update(selectedSnapshots, (current) => [...current, [...selectedNames]]);
        ...
        expect(yield* Ref.get(selectedSnapshots)).toEqual([[], ['batch', 'simplify']]);
