...
const makeTestLayer = (entries: FsEntries) => {
  const baseLayer = Layer.mergeAll(makeSkillFsLayer(entries), TestStowConfig, TestPath);
...
`,
    ...
    return Effect.gen(function* () {
      ...
      expect(preview.toCreate).toEqual(['~/.claude/skills/batch', '~/.agents/skills/batch']);
